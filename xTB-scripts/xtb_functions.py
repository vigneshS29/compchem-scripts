...
def xtb_energy(xyz_file,charge=-1,unpair=0,niter=100,accuracy=1.0,namespace='xTB',workdir='.',method='sp',etemp=None):
...
def xtb_egap(xyz_file,charge=0,unpair=0,niter=100,accuracy=1.0,namespace='xTB',workdir='.',method='sp',etemp=None):
    current_dir = os.getcwd()
    # change to xTB working directory
    os.chdir(workdir)
    if etemp is None:
    ...
    with open("{}_xTB-Energy.txt".format(namespace),"r") as g:
        for lc,lines in enumerate(g):
            fields=lines.split()
            if len(fields)==6 and fields[1] =="HOMO-LUMO" and fields[2] == "GAP":
                Egap = float(fields[3])
